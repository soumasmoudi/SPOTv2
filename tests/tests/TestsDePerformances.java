...
public class TestsDePerformances {
	...
	long[] time_Sig_Verify_Agg = new long[nb_execs];
	long[] time_CCM_Verify_Agg = new long[nb_execs];
	...
	*/
	...
	// Prepare a list of 100 signed messages
	
	for (int k = 0; k < 99; k++) {
		Element Ctc = user.set_CCM_U();
		Element[] ssig = server.s_PSign_S(Ctc);
		Contact psig = proxy.P_sign_P(ssig[0], user.getIDu());
		user.add_contact(new ContactSigned(psig, ssig[1]));
	}
	/**
	* Sig_Verify_aggregation
	*/
	start = System.nanoTime();
	ha.Sig_Verify_aggregation(user.getContact_list(), gm.getDatabaseGM().getCrs());
	end = System.nanoTime();
	time_Sig_Verify_Agg[i] = (long) (end - start);
	
	/**
	* CCM_Verify_aggregation
	*/
	start = System.nanoTime();
	ha.CCM_Verify_aggregation(server, user);
	end = System.nanoTime();
	time_CCM_Verify_Agg[i] = (long) (end - start);
	...
	System.out.println("Sig_Verify_Agg" + "\nmean in ms   " +
			String.valueOf(Mean.mean(time_Sig_Verify_Agg) / 1000000) + "\nmedian in ms   "
			+ String.valueOf(Median.median(time_Sig_Verify_Agg) / 1000000) +
			"\nstandard deviation in ms   " +
			String.valueOf(StandardDeviation.standardDev(time_Sig_Verify_Agg) / 1000000));
			
			System.out.println("CCM_Verify_Agg" + "\nmean in ms   " +
			String.valueOf(Mean.mean(time_CCM_Verify_Agg) / 1000000) + "\nmedian in ms   "
			+ String.valueOf(Median.median(time_CCM_Verify_Agg) / 1000000) +
			"\nstandard deviation in ms   " +
			String.valueOf(StandardDeviation.standardDev(time_CCM_Verify_Agg) / 1000000));
